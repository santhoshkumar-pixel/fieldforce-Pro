...
import { Camera, FileVideo, Send, Upload, X } from "lucide-react";

export default function ReachedModal({ open, ticketId, onClose, onSubmit }) {
 ...
 {/* Header */}
 <div className="flex items-center justify-between border-b border-slate-800 bg-gradient-to-r from-violet-900/40 to-slate-900 px-6 py-4">
 ...
 <div>
 <h2 className="text-base font-semibold text-white">Site Reached</h2>
 <p className="text-xs text-slate-400">Upload proof & add a note</p>
 </div>
 </div>
 <button
 type="button"
 ...
 className="flex w-full flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-slate-700 bg-slate-800/40 py-8 text-slate-400 hover:border-violet-500/50 hover:bg-violet-500/5 hover:text-violet-400"
 >
 ...
 placeholder="Describe the situation on site..."
 rows={3}
 className="w-full resize-none rounded-xl border border-slate-700 bg-slate-800/60 px-4 py-3 text-sm text-slate-100 placeholder-slate-500 outline-none focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 "
 />
 </div>
 </div>
 ...
 className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-2 text-sm font-semibold text-white hover:bg-violet-500 "
 >
 <Send className="h-4 w-4" />
 Submit & Set to Review
 </button>
 </div>
 </div>
 </div>
